...
ISR(TCA0_OVF_vect) {
  ...
  if (edgecount == 0) {
    ...
    /*
    uart.DF("%u", current_bit);
    if (bitout++%8 == 7) uart.D(" ");
    */
  }
...
ISR(USART0_RXC_vect) {
  ...
  if (in != '\n') {
    // backspace
    if (in == '\r') {
      UART::rollbefore(&buff_p, 20);
    }
    // any key
    else {
      buff[buff_p] = in;
      UART::rollover(&buff_p, 20);
    }
  }
      ...
      case CMD_STATE::RESET:
        {
          ...
          send_packet(packets, 2);
          uart.ARR("buffer", buffer, BUFF_SIZE_BYTES);
          uart.DF("in: %u out: %u\n", buff_pos_in, buff_pos_out);
          */
          // 25 reset packets
