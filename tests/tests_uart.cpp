...
Test_Result tests_uart() {
  ...
  // u2c
  len = uart.u2c(buf, 121, 1);
  number_of_passed += validate("u2c (12.1)", "12.1", buf);
  number_of_passed += validate("u2c len", 4, len);

  len = uart.u2c(buf, 123, 0);
  number_of_passed += validate("u2c (123)", "123", buf);
  number_of_passed += validate("u2c len", 3, len);

  len = uart.u2c(buf, 12, 2);
  number_of_passed += validate("u2c (0.12)", "0.12", buf);
  number_of_passed += validate("u2c len", 4, len);
  number_of_tests += 6;

  // uart_sec2human
  len = uart.sec2human(buf, 10);
  number_of_passed += validate("sec2human (10s)", "10s", buf);

  len = uart.sec2human(buf, 12253);
  number_of_passed += validate("sec2human (03h24m13s)", "03h24m13s", buf);

  len = uart.sec2human(buf, 359);
  number_of_passed += validate("sec2human (05m59s)", "05m59s", buf);

  uint32_t send_data_at = 3691486;
  uint32_t now = 17774;
  len = uart.sec2human(buf, (((send_data_at - now)) * (7 + 1)) / 32768);
